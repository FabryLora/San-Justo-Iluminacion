...
@section('content')
    ...
    <div class="w-[1224px] mx-auto min-h-[40vh] py-20 flex flex-col gap-14" x-data="{ 
        ...
        <div class="flex flex-col">
            ...
            <div class="text-[20px]! break-words max-w-[863px]">
                {!! request('lang') == 'en' ? $trabaja->text_en : $trabaja->text_es !!}
            </div>
        </div>

        <div class="w-full flex justify-center">
            <form x-ref="workForm" @submit.prevent="submitForm" class="grid grid-cols-2 gap-5">
                @csrf
                <!-- Mostrar mensaje de error si existe -->
                <div x-show="error" x-transition
                    class="col-span-2 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative"
                    role="alert">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" id="nombre" name="nombre" class="border h-[49px] w-[500px] rounded-sm pl-4" required>
                </div>
                <div class="flex flex-col gap-2">
                    <label for="apellido">{{__('Apellido') . " *"}}</label>
                    <input type="text" id="apellido" name="apellido" class="border h-[49px] w-[500px] rounded-sm pl-4"
                        required>
                </div>
                <div class="flex flex-col gap-2">
                    <label for="telefono">{{__('Telefono') . " *"}}</label>
                    <input type="text" id="telefono" name="telefono" class="border h-[49px] w-[500px] rounded-sm pl-4"
                        required>
                </div>
                <div class="flex flex-col gap-2">
                    <label for="email">{{__('Correo electrónico')}} *</label>
                    <input type="email" id="email" name="email" class="border h-[49px] w-[500px] rounded-sm pl-4" required>
                </div>
                <div class="flex flex-col gap-2">
                    <label for="archivo">{{__("Adjunta tu CV") . " *"}}</label>
                    <div class="flex flex-row border h-[49px] w-[500px] rounded-sm items-center justify-between px-4">
                        <p class="text-[16px] text-[#7C7C7C]">{{__('Podés cargar PDF, Word, PNG de hasta 2MB')}}</p>
                        <label class="cursor-pointer" for="archivo">
                ...
                <button type="submit" :disabled="isLoading" :class="isLoading ? 'opacity-50 cursor-not-allowed' : ''"
                    class="w-full text-white bg-primary-orange rounded-sm h-[49px] self-end text-[14px] font-medium">
        ...
        <div x-show="showModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
            ...
            <div x-show="showModal" x-transition:enter="transition ease-out duration-300"
                ...
                class="bg-white rounded-sm w-[600px] h-[297px] relative">
                ...
                <!-- Contenido del modal -->
                <div class="text-center  flex justify-center items-center flex-col gap-4 w-[600px] h-[297px]">
                    <!-- Ícono de éxito -->


                    <h3 class="text-[30px] font-semibold font-custom!">
